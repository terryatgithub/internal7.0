...
function buttonInitBefore() {
	...
	$("#salvage").unbind("focus").bind("focus", function() {
		console.log("salvage得到焦点");
		$("#lylgif").css("display","block");
	});
	$("#salvage").unbind("blur").bind("blur", function() {
		console.log("salvage失去焦点");
		$("#lylgif").css("display","none");
	});
	$("#dreamFly").unbind("focus").bind("focus", function() {
		console.log("dreamFly得到焦点");
		$("#gbgif").css("display","block");
	});
	$("#dreamFly").unbind("blur").bind("blur", function() {
		console.log("dreamFly失去焦点");
		$("#gbgif").css("display","none");
	});
	choiceClickFunc();
	professClickFunc();
...
function buttonInitAfter() {
	...
	$(".coocaa_btn3").unbind("itemClick").bind("itemClick", function() {
		...
		showProfessCard(1,_curTime, _curText, _curName, _curCardId);
...
function backButtonFunc() {
	...
	if(page6 == "block") {
		...
		if(page2 == "block" || page3 == "block" || page4 == "block" || page5 == "block") {
			...
			if (_curBtnId == "salvage") {
				$("#salvage").trigger("focus");
			}
			if (_curBtnId == "dreamFly") {
				$("#dreamFly").trigger("focus");
			}
		} else {
...
function sendPrizes(awardid, rememberId, type, userkeyid, openid) {
	...
	var ajaxTimeoutFive = $.ajax({
		...
		success: function(data) {
			...
			if(data.code == "50100") {
				if(type == 14) {
					showProfessCard(0,data.data.WishInfo.createTime, data.data.WishInfo.wishMsg, data.data.WishInfo.wishName);
				}
				if(document.getElementById("awardPage").style.display == "block") {
...
//展示愿望卡
function showProfessCard(area,time, text, name, cardid) {
	console.log(time+"--"+text+"--"+name+"--"+cardid);
	time = time.substr(0, 10);
	var _curTextLength = text.length;
	console.log(area == "1");
	if(_curTextLength < 22) {
		if(area == "1"){
			$("#category1").css("background-image", "url(images/dialog/profess/1.png)");
		}else{
		...
		if(area == "1"){
			$("#category1").css("background-image", "url(images/dialog/profess/2.png)");
		}else{
		...
		if(area == "1"){
			$("#category1").css("background-image", "url(images/dialog/profess/3.png)");
		}else{
		...
		if(area == "1"){
			$("#category1").css("background-image", "url(images/dialog/profess/4.png)");
		}else{
...
function webBtnClickLog(f, dateObj) {
