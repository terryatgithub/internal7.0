...
cordova.define("coocaaosapi", function(require, exports, module) {
    ...
    //包名类名方式启动
    CoocaaOSApi.prototype.startUserSettingAndFinish2 = function(pkgname,classname, success, error) {
        argscheck.checkArgs('ssff', 'CoocaaOSApi.startUserSettingAndFinish', arguments);
        startapp.check(pkgname, function(message) { /* success */
            startapp.start([
                [pkgname, classname],
                // [{ 'needFinish': true }]
            ], success, error);
    ...
    CoocaaOSApi.prototype.startOrCreateDownloadTask = function(downloadurl, md5, title, packageName, appID, iconUrl, success, error) {
        ...
        startapp.check(packageName, function(checksuccess) {
            ...
            exec(success, error, '', 'createDownloadTask', [{ 'url': downloadurl }, { 'md5': md5 }, { 'title': title }, { 'pkg': packageName }, { 'appid': appID }, { 'icon': iconUrl }]);
        });
    ...
    CoocaaOSApi.prototype.startOrCreateDownloadTask3 = function(downloadurl, md5, title, packageName, appID, iconUrl, success, error) {
        argscheck.checkArgs('ssssssff', 'CoocaaOSApi.startOrCreateDownloadTask', arguments);
    ...
    */
    ...
    CoocaaOSApi.prototype.purchaseOrder2 = function(appcode, Tradeid, ProductName, SpecialType, amount, ProductType, payAction, cmd, token, tel, test, test2, success, error) {
    ...
    CoocaaOSApi.prototype.startMovieWebviewTwoPage = function(url, success, error) {
        ...
        startapp.start([
            ["action", "coocaa.intent.movie.popview"],
            [{ "url": url }]
        ], success, error);
    }
    ...
    CoocaaOSApi.prototype.startNormalAction = function(url, success, error) {
        ...
        startapp.start([
            ["action", "coocaa.intent.vip.center.card"],
            [{ "appcode": url }]
        ], success, error);
    }
    ...
    //启动我的优惠券
    CoocaaOSApi.prototype.startMyCoupon = function(sign, openId, appId, businessLine, success, error) {
        argscheck.checkArgs('ssssff', 'CoocaaOSApi.startAllCoupon', arguments);
        startapp.start([
            ["action", "coocaa.intent.action.MYCOUPON"],
            [{ "sign": sign }, { "openId": openId }, { "appId": appId }, { "businessLine": businessLine }]
        ], success, error);
    }
    ...
    //包名方式启动
    CoocaaOSApi.prototype.startUserSettingAndFinish4 = function(pkgname, success, error) {
    ...
    //    argscheck.checkArgs('ssssff', 'CoocaaOSApi.startParamAction', arguments);
        startapp.start([
            [bywhat, byvalue], sources
        ], success, error);
    }
    ...
    //默认跳转
    CoocaaOSApi.prototype.startDefaultZone = function(openId,success,error){
       startapp.start([["action", "coocaa.intent.action.ALLCOUPON"],[{'sign':'5168fce41de811e78d6500163e022eda'},{'appId':'CC0003'},{'businessLine':'normal'},{'openId':openId}]], success,error);
    }
    //包名方式启动
    CoocaaOSApi.prototype.startUserSettingAndFinish4 = function(pkgname, success, error) {
      argscheck.checkArgs('sff', 'CoocaaOSApi.startUserSettingAndFinish', arguments);
    ...
    //启动传参action
    // 包名、版本号、startActivity、action、action名、拓展参数[{key1:"value1"},{key2:"value2"}]
    CoocaaOSApi.prototype.openAppByAction = function(bywhat, byvalue, success, error) {
        // sources = JSON.parse(sources);
        console.log("启动传参action")
           argscheck.checkArgs('ssff', 'CoocaaOSApi.openAppByAction', arguments);
        startapp.start([
            [bywhat, byvalue]
        ], success, error);
    }
    ...
    CoocaaOSApi.prototype.startOtherApp = function(pkgname,uri,success,error){
        argscheck.checkArgs('ssff','CoocaaOSApi.startOtherApp',arguments);      
            startapp.start([["action", "android.intent.action.VIEW",pkgname,"",
                uri]], success,error);        
    }

    //教育版面
    CoocaaOSApi.prototype.openEduPage = function(business_type,cmd,jumpToPage,success, error) {
        startapp.start([
            ["action", "coocaa.intent.vip.center"],
            [{ "business_type": business_type },{ "cmd": cmd },{ "jumpToPage": jumpToPage }]
        ], success, error);
    }
