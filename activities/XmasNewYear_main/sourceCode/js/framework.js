...
var awardurl = "http://beta.webapp.skysrt.com/yuanbo/test/XmasNewYear2018/myaward.html?from=home";
...
function buttonInitBefore() {
	...
	$("#getRedBag").unbind("itemClick").bind("itemClick", function() {
		var _curType = $("#getRedBag").attr("curType");
		var awardId2 = $("#getRedBag").attr("awardId");
		var rememberId2 = $("#getRedBag").attr("rememberId");
		var userKeyId2 = $("#getRedBag").attr("userKeyId");
		var awardTypeId2 = $("#getRedBag").attr("awardTypeId");
		var activeId2 = $("#getRedBag").attr("activeId");
		var awardName2 = $("#getRedBag").attr("awardName");
		var awardTime2 = $("#getRedBag").attr("awardTime");

		if(_curType == 7) {
			if(_loginstatus == "false") {
				console.log("点击了收下红包+启登录");
				startAndSendLog(2);
			} else {
				getRedPacketsQrcode(activeId2, rememberId2, userKeyId2, "redQrcode");
				...
				if(goldHouseIsOpen == 2) {
					sentLog("new_player_gift_button_click", '{"page_name":"圣诞小屋页面","activity_name":"双旦活动-圣诞小屋","page_type":"黄金小屋已开启","button_name":"收下红包"}');
					_czc.push(['_trackEvent', '双旦活动-圣诞小屋页面', '新手礼包点击', '黄金小屋已开启+收下红包', '', '']);
				} else {
					sentLog("new_player_gift_button_click", '{"page_name":"圣诞小屋页面","activity_name":"双旦活动-圣诞小屋","page_type":"黄金小屋未开启","button_name":"收下红包"}');
			...
			if(_loginstatus == "false") {
				...
				if(goldHouseIsOpen == 2) {
					...
					sentLog("new_player_gift_button_click", '{"page_name":"圣诞小屋页面","activity_name":"双旦活动-圣诞小屋","page_type":"黄金小屋已开启","button_name":"使用红包"}');
					_czc.push(['_trackEvent', '双旦活动-圣诞小屋页面', '新手礼包点击', '黄金小屋已开启+使用红包', '', '']);
				} else {
					randBtnFunc();
					sentLog("new_player_gift_button_click", '{"page_name":"圣诞小屋页面","activity_name":"双旦活动-圣诞小屋","page_type":"黄金小屋未开启","button_name":"使用红包"}');
					_czc.push(['_trackEvent', '双旦活动-圣诞小屋页面', '新手礼包点击', '黄金小屋未开启+使用红包', '', '']);
				}
				sendPrizes(awardId2, rememberId2, userKeyId2, awardTypeId2, activeId2, _qsource);
			}
		} else if(_curType == 2) {
			if(_loginstatus == "false") {
				console.log("点击了收下实物奖+启登录");
				startAndSendLog(2);
			} else {
				$("#firstLoadAward").css("display", "none");
				$("#firstSendEntity").css("display", "block");
				$("#firstEnName").html("奖品名称:" + awardName2);
				$("#firstEnTime").html("获奖时间:" + awardTime2);
				var enstr = enurl + "activeId=" + activeId2 + "&rememberId=" + rememberId2 + "&userKeyId=" + userKeyId2 + "&access_token=" + _accessToken;
	...
	$(".homeBtns").unbind("focus").bind("focus", function() {
		...
		if(_fIndex == 5) {
			...
			if(goldHouseIsOpen == 3) {
				$("#packDialog").html("活动已结束圣诞小屋即将消失<br/>快去【我的礼物】页面把礼物都领走吧");
			} else if(goldHouseIsOpen == 1) {
				$("#packDialog").html("帮忙打包圣诞礼物，<br/>可获得1个铃铛哦<br/>按【下键】选择商品");
				if(_packageOver) {
					$("#packDialog").html("今天打包次太多了，再完成打包圣诞爷爷不会给奖励啦~<br/>试试其他任务吧");
		...
		if(_fIndex == 3) {
			...
			$("#restDialog").css("line-height", "36px");
			if(goldHouseIsOpen == 3) {
				$("#restDialog").html("活动已结束圣诞小屋即将消失<br/>快去【我的礼物】页面把礼物都领走吧");
				$("#restDialog").css("line-height", "27px");
			} else if(goldHouseIsOpen == 1) {
				$("#restDialog").html("麋鹿宝宝好无聊，<br/>带它出去散步可获得1个铃铛哦<br/>按【确定】出发");
			} else if(goldHouseIsOpen == 2) {
				if(_elkOver) {
					$("#restDialog").html("今日散步次数太多了，<br/>麋鹿宝宝累了，<br/>去做其他任务吧");
				} else {
					$("#restDialog").html("麋鹿宝宝好无聊，<br/>带它出去散步可获得1个铃铛哦<br/>按【确定】出发");
				}
			}
			...
			$("#goldDialog").css("line-height", "35px");
			if(goldHouseIsOpen == 3) {
				$("#goldDialog").html("活动已结束圣诞小屋即将消失<br/>快去【我的礼物】页面<br/>把礼物都领走吧");
			} else if(goldHouseIsOpen == 1) {
				$("#goldDialog").html("31日0点开启！<br/>获得5个铃铛将有机会<br/>获得价值5万的锦鲤礼包");
			} else if(goldHouseIsOpen == 2) {
				if(_remainingNumber < 5) {
					$("#goldDialog").html("黄金小屋已开启，<br/>请先去做任务获得5个铃铛<br/>再来召唤锦鲤和礼物吧~");
				} else {
					if(_remainKoiNumber == 1) {
						if(_isLessFivs == 1) {
							$("#goldDialog").html("黄金小屋即将关闭<br/>酷开锦鲤还在等你，<br/>快来把它带走吧，<br/>按“确定”键抽奖");
							$("#goldDialog").css("line-height", "27px");
						} else {
							var items = ['黄金小屋已开启，<br/>快来参与抽奖，<br/>百万豪礼等你拿，<br/>按“确定”键抽奖', '黄金小屋已开启，<br/>快按【确定】尝试召唤锦鲤，<br/>每次召唤将消耗5个铃铛哦~'];
							var item = items[Math.floor(Math.random() * items.length)];
							$("#goldDialog").html(item);
							if(Math.floor(Math.random() * items.length) == 2) {
								$("#goldDialog").css("line-height", "27px");
							}
						}
					} else {
						$("#goldDialog").html("iPhone XS、创维电视等礼物<br/>还在黄金小屋！<br/>按【确定】召唤，<br/>每次召唤将消耗5个铃铛哦~");
						$("#goldDialog").css("line-height", "27px");
					}
				}
			}
			...
			if(goldHouseIsOpen == 3) {
				$("#purchaseDialog").html("活动已结束<br/>圣诞小屋即将消失<br/>快去【我的礼物】页面<br/>把礼物都领走吧");
			} else if(goldHouseIsOpen == 1) {
				$("#purchaseDialog").html("帮忙采购圣诞礼物<br/>圣诞爷爷会奖励5个铃铛<br/>还有现金返利<br/>快按【确定】进入");
			} else if(goldHouseIsOpen == 2) {
				$("#purchaseDialog").html("帮忙采购圣诞礼物<br/>圣诞爷爷会奖励5个铃铛<br/>还有现金返利<br/>快按【确定】进入");
			}
		}
	...
	$("#packHome").unbind("itemClick").bind("itemClick", function() {
		...
		if(goldHouseIsOpen == 1) {
			...
			sentLog("christmas_house_page_button_click", '{"page_name":"圣诞小屋页面","activity_name":"双旦活动-圣诞小屋","page_type":"黄金小屋已结束","button_name":"打包小屋"}');
			_czc.push(['_trackEvent', '主活动页面', '主活动页面按钮点击', '黄金小屋已结束+打包小屋', '', '']);
		}
	});
	$("#goldHome").unbind("itemClick").bind("itemClick", function() {
		...
		if(goldHouseIsOpen == 1) {
			...
			if(_remainingNumber < 5) {
				...
				document.getElementById("curBellNum").innerHTML = _remainingNumber;
			...
			sentLog("christmas_house_page_button_click", '{"page_name":"圣诞小屋页面","activity_name":"双旦活动-圣诞小屋","page_type":"黄金小屋已结束","button_name":"黄金小屋"}');
			_czc.push(['_trackEvent', '主活动页面', '主活动页面按钮点击', '黄金小屋已结束+黄金小屋', '', '']);
		}
	});
	...
	$("#activeRule").unbind("itemClick").bind("itemClick", function() {
		...
		if(goldHouseIsOpen == 1) {
			...
			sentLog("christmas_house_page_button_click", '{"page_name":"圣诞小屋页面","activity_name":"双旦活动-圣诞小屋","page_type":"黄金小屋已结束","button_name":"活动规则"}');
			_czc.push(['_trackEvent', '主活动页面', '主活动页面按钮点击', '黄金小屋已结束+活动规则', '', '']);
		}
	...
	$("#myAward,#hasEndBtn,#hasCloseBtn,#koi2btn2").unbind("itemClick").bind("itemClick", function() {
		...
		coocaaosapi.startNewBrowser3(awardurl + "&actEnd=" + _bActivityEnd + "&awardToast=false", function() {
			console.log("OK");
		}, function() {});
	...
	$("#koibtn2").unbind("itemClick").bind("itemClick", function() {
		...
		if(_kAwardTypeId == 2 || _kAwardTypeId == 7) {
			console.log("跳转我的礼物");
			sentLog("main_reward_page_show", '{"page_name":"我的礼物","activity_name":"双旦活动-我的礼物页面","last_page_name":"圣诞小屋页面"}');
			_czc.push(['_trackEvent', '双旦活动-我的礼物', '我的礼物页面曝光', '', '', '']);
	...
	$("#packList").unbind("itemClick").bind("itemClick", function() {
		...
		if(goldHouseIsOpen == 1) {
			...
			sentLog("christmas_house_page_button_click", '{"page_name":"圣诞小屋页面","activity_name":"双旦活动-圣诞小屋","page_type":"黄金小屋已结束","button_name":"打包清单"}');
			_czc.push(['_trackEvent', '主活动页面', '主活动页面按钮点击', '黄金小屋已结束+打包清单', '', '']);
			sentLog("pack_list_page_show", '{"page_name":"打包清单页面","activity_name":"双旦活动-打包任务页面","last_page_name":"圣诞小屋页面","page_type":"黄金小屋已结束"}');
			_czc.push(['_trackEvent', '主活动页面', '打包清单页面曝光', '黄金小屋已结束', '', '']);
		}
	});
...
function randBtnFunc() {
	var idArray = ['restArea', 'packHome', 'purchaseHome'];
	var idName = idArray[Math.floor(Math.random() * idArray.length)];
	console.log(idName);
...
//			$("#packHome").trigger("focus");
			...
			var url = "http://beta.webapp.skysrt.com/yuanbo/test/XmasNewYear2018/myaward.html?actEnd=" + _bActivityEnd + "&awardToast=false&from=home";
			coocaaosapi.startNewBrowser4(url, function(success) {
				console.log("startNewBrowser4 success");
			}, function(err) {
				console.log("startNewBrowser4 error")
			});
		} else {
	...
	$(".coocaa_btn_pack").bind("focus", function() {
		var _Lindex = $(".coocaa_btn_pack").index($(this));
		
		var parentId = $(this).parent().attr("id");
		var itemHeight = 398; //398px
//		var myScrollTopValue = Math.floor(parseInt(_Lindex/5)) *(398);
		...
		success: function(data) {
			...
			if(data.code == 50001) {
				...
				if(num1 == 1) {
					...
					sentLog("christmas_house_page_show", '{"page_name":"圣诞小屋页面","activity_name":"双旦活动-圣诞小屋","page_type":"黄金小屋已结束"}');
					_czc.push(['_trackEvent', '圣诞小屋曝光', '圣诞小屋页面', '黄金小屋已结束', '', '']);
				}
				$("#bellnum").val("X0");
				showGoldHousePlace(3, 6);
				...
				if(num3 == 0) {
					...
					if(cSysTime < goldBeginTime) {
						console.log("黄金屋活动未开始");
						goldHouseIsOpen = 1;
					} else {
						console.log("黄金屋活动已开始");
						goldHouseIsOpen = 2;
						if(goldEndTime - cSysTime < 18000000) {
							_isLessFivs = 1;
						} else {
							_isLessFivs = 0;
						}
...
//页面首次加载的弹窗逻辑
function showInitDialog(dataObj, num4) {
	...
	if(dataObj.chanceResult.firstIn == 0) {
		...
		if(dataObj.chanceResult.chanceSource.length == 1) {
			...
			if(dataObj.chanceResult.chanceSource[0] > 0 && dataObj.chanceResult.chanceSource[0] < 4) {
				...
				if(_awardtype == 2) {
					...
					$("#redAwardName").html(dataObj.chanceResult.rememberModel.awardName);
					$("#redAwardInfo").html(dataObj.chanceResult.rememberModel.awardInfo.bonus);
					$("#thanks_btn1 .btnName").html("收下红包");
					$("#thanks_btn1").attr("activeId", dataObj.chanceResult.rememberModel.lotteryActiveId);
					$("#thanks_info3").css("display", "block");
					$("#thanks_info3").html('*目前累计获得红包<span style="color:#fff642">' + dataObj.chanceResult.rememberModel.awardInfo.bonus + '</span>元');
				}
		...
		if(num4 == 1) {
			if(document.getElementById("packGoodsBox").style.display == "block") {
				...
				$("#packGoodsDiv").css("display", "block");
	...
	if (dataObj.chanceResult.giftBell) {
		$("bulletinBoard").html("圣诞老人送你3个铃铛");
		setTimeout(changeThebulletinInfo(), 5000);
	}
}
function changeThebulletinInfo(){
	$("bulletinBoard").css("display","none");
	$("bulletinBoard").html("修改文案");
	...
	var ajaxTimeoutTwo = $.ajax({
		...
		success: function(data) {
			...
			if(data.code == 50001) {
				...
				$("#getRedBag").attr("awardId", fAwardId);
				$("#getRedBag").attr("rememberId", fRememberId);
				$("#getRedBag").attr("userKeyId", fUserKeyId);
				$("#getRedBag").attr("awardTypeId", fAwardTypeId);
				$("#getRedBag").attr("activeId", fActiveId);
				$("#getRedBag").attr("awardName", fAwardName);
				$("#getRedBag").attr("awardUrl", fAwardUrl);
				$("#getRedBag").attr("awardTime", fAwardTime);
				...
				if(fAwardTypeId == 7) {
					...
					$("#getRedBag .imgBlur").attr("src", "images/dialog/getRed_blur.png");
					$("#getRedBag .imgFocus").attr("src", "images/dialog/getRed_focus.png");
				...
				$("#getRedBag").attr("curType", fAwardTypeId);
...
//获取已打包商品
function getPackedGoodsList(num,num2) {
		...
		success: function(data) {
			...
			if(data.code == 0) {
				if(data.data == true) {
					...
					if(!_packageOver) {
						setTimeout(function() {getPackedGoodsList(0,0);}, 1000);
						setTimeout(function() {getDoneWorkAward(3);}, 1000);
						//getDoneWorkAward(3); //0-登录任务 3-打包任务
					} else {
						setTimeout(function() {getPackedGoodsList(1,0);}, 1000);
					}
					if(goldHouseIsOpen == 2) {
...
function showWorkDialog(dataObj, num) {
	...
	//var _oAwardName = dataObj.rememberModel.awardName;
	...
	if(_awardtype == 2) {
		...
		$("#thanks_coupon").css("display", "none");
		$("#thanks_red").css("display", "block");
		$("#redAwardName").html(dataObj.rememberModel.awardName);
	...
	$.ajax({
		...
		success: function(data) {
				...
				if(data.data.koiNews.result.length > 0) {
					...
					for(var i = 0; i < data.data.koiNews.result.length; i++) {
						...
						if(data.data.koiNews.result[i].nickName == "") {
							...
							var username = data.data.koiNews.result[i].nickName.length > 7 ? data.data.koiNews.result[i].nickName.substr(0, 6) + "..." : data.data.koiNews.result[i].nickName;
							list.innerHTML = "恭喜" + data.data.koiNews.result[i].province + " 用户" + username + " 成为" + data.data.koiNews.result[i].queue + "号锦鲤";
						}
						box.appendChild(list);
					}
					showAwardlist("#koiNews", "#koiNewsul");

					if(data.data.koiNews.result.length == 2) {
						_remainKoiNumber = 0;
					}
				} else {
			...
			for(var i = 0; i < data.data.fakeNews.length; i++) {
				...
				//          	if (_curNickName.length>4) {
				//          		_curNickName = _curNickName.substring(0,4)+"...";
				//          	}
				//          	if (_curAwardName.length>9) {
				//          		_curAwardName = _curAwardName.substring(0,9)+"...";
				//          	}
				var list = document.createElement("li");
				list.innerHTML = _curNickName + " 带麋鹿散步 获得" + _curAwardName;
				box.appendChild(list);
			}
...
function showThisAwardDialog(awardObj) {
	...
	$("#machine").stop(true, true).animate({
		scrollTop: 0
	}, {
		duration: 0,
		easing: "swing",
	...
	if(_cawardTypeId == 2) {
		...
		$("#getKoiInfo").html('获得<span style="color: #ffe82a">' + _cawardName + '</span>');
		...
		$("#getKoi").css("display", "block");
		$("#getKoiInfo").html('获得<span style="color: #ffe82a">' + _bouns1 + '</span>元现金!');
		$("#redImgShow").css("display", "block");
		$("#entityImgShow").css("display", "none");
		$("#couponImgShow").css("display", "none");
		$("#koibtn2 .btnName").html("我的礼物");
		map = new coocaakeymap($(".coocaa_btn2"), document.getElementById("koibtn1"), "btn-focus", function() {}, function(val) {}, function(obj) {});
		getRedPacketsQrcode(_cactiveId, _crememberId, _cuserKeyId, "redQrcodePart");
		...
		var enstr = enurl + "activeId=" + _cactiveId + "&rememberId=" + _crememberId + "&userKeyId=" + _cuserKeyId + "&access_token=" + _accessToken;
		drawQrcode("koiMouldImg2", enstr, 190);
	}
}

function startAndSendLog(num){
	_startLogin = num; //2表示领取奖励时启登录
	startLoginFlag = true;
	startLogin(needQQ);
	sentLog("landing_page_show", '{"page_name":"双旦活动登录弹窗","activity_name":"双旦活动-圣诞小屋","last_page_name":"圣诞小屋页面"}');
	...
	$("._jrgoods").unbind("itemClick").bind("itemClick", function() {
		...
		if(_thisType == "packnormal") {
			...
			if(goldHouseIsOpen == 1) {
				...
				sentLog("pack_list_page_show", '{"page_name":"打包清单页面","activity_name":"双旦活动-打包任务页面","last_page_name":"采购小屋页面","page_type":"黄金小屋已结束"}');
				_czc.push(['_trackEvent', '打包清单页面', '打包清单页面曝光', '黄金小屋已结束', '', '']);
			}
...
function onResumeFunc() {
	...
	if (startLoginFlag&&changeLoginFlag) {
		...
		if(document.getElementById("firstLoadAward").style.display == "block") {
			$("#getRedBag").trigger("itemClick");
			_startLogin = 0;
		}
