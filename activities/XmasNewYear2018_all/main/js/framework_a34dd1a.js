...
var _lotteryid = 99; //抽奖活动id
var _buyActiveId = 98; //采购活动id
...
var packlisturl = "http://api.tvshop.coocaa.com/cors/tvCartAPI/packGoodsList";
var packgoodUrl = "http://api.tvshop.coocaa.com/cors/tvCartAPI/addCartFromAct";
var goodsCouponUrl = "http://api.tvshop.coocaa.com/cors/tvUsersAPI/goodsCoupon";
...
function showSandanInfo(dataObj,num){
	...
	if(goldHouseIsOpen == 1){
		console.log("------------------------1");
		$("#bulletinBoard").css("background-image","url(images/pop/new1.png)");
	}else if(goldHouseIsOpen == 2){
	...
	if (num == 1) {
		setTimeout(changeThebulletinInfo,10000);
	}else{
			...
			if(data.code == 0) {
				if(data.data == true) {
					...
					if(!_packageOver) {
						...
						setTimeout(function() {getPackedGoodsList(1,0,0);}, 1000);
					}
					if(goldHouseIsOpen == 2) {
...
function showWorkDialog(dataObj, num) {
	...
	if (num == 4) {
		...
		if(_awardtype == 2) {
			...
			$("#redAwardInfo").html(_showBonus + "元");
			$("#thanks_btn1 .btnName").html("收下红包");
		}
		if(num == 1 || num == 2) {
			$("#thanks_btn2 .btnName").html("关闭");
	...
	$.ajax({
		...
		success: function(data) {
			...
			for(var i = 0; i < data.data.fakeNews.length; i++) {
				...
				var list = document.createElement("li");
				var items = [' 带麋鹿散步 ',' 打包礼物 ',' 采购商品 '];
				var item = items[Math.floor(Math.random() * items.length)];
				list.innerHTML = _curNickName + item + _curAwardName;
				box.appendChild(list);
			}
...
function showThisAwardDialog(awardObj) {
	...
	if(_cawardTypeId == 2) {
		...
		$("#getDrawEnName").html("奖品名称"+_cawardTime);
